...
namespace dpaw_alerts.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            Dsearch.Filter = string.Format("(&(samaccountname={0})(objectClass=person))", model.UserName);
            ...
            if (model.SecurityCode == Session["randomStr"].ToString())
            {
                ...
                {
                    //log user activity
                    LogError.LogUserActivity(model.UserName, "Login success", "Login");
                    // Default authentication block

                    string pwd = "%4$FD@Aadfsd*";
                    // This doesn't count login failures towards account lockout
                    // To enable password failures to trigger account lockout, change to shouldLockout: true
                    var result = await SignInManager.PasswordSignInAsync(model.UserName, pwd, model.RememberMe, shouldLockout: false);
                    switch (result)
                    {
        ...
        public async Task<ActionResult> UserProfile()
        {
            ...
            var user = await _users.GetUserByNameAsync(currentUser);
