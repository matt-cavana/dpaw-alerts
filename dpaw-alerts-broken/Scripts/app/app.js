...
$(function	()	{
    ...
    $('div.main-menu ul li a').each(function () {
        if (str.indexOf(this.href.toLowerCase()) > -1) {
            ...
            $(this).parent('li').addClass("active"); //hightlight parent tab
        }
    });
	...
	$(window).resize(function (e) {
		...
		if (Modernizr.mq('(min-width: 768px)') && Modernizr.mq('(max-width: 868px)')) {
			...
			$('.main-menu').find('.submenu').removeAttr('style');
		    //resize the button
			$('#btnReset').addClass('btn-xs').removeClass('btn-sm');
			$('#btnSearch').addClass('btn-xs').removeClass('btn-sm');
			$('a.alertInfo').addClass('btn-xs').removeClass('btn-sm');
			...
			if($('#wrapper').hasClass('window-resize'))	{
				...
				$('.main-menu').find('.submenu').removeAttr('style');
			    //resize the button
				$('#btnReset').addClass('btn-sm').removeClass('btn-xs');
				$('#btnSearch').addClass('btn-sm').removeClass('btn-xs');
				$('a.alertInfo').addClass('btn-sm').removeClass('btn-xs');
			}
		}
		else	{
			$('#wrapper').removeClass('sidebar-mini window-resize');
